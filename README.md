...
1. 修复了安卓端每周更新提醒所需的本地通知调度配置缺失问题，并补充了对应回归测试。
2. 修复了 iOS 在通知栏点击提醒后只返回主界面的问题，现可正确跳转到对应番剧详情页，并补强了通知点击链路的调试能力。
3. 重构了更新提醒规则：现在只有“在看”状态下才能开启和保留固定时间提醒；当追番状态切换为“想看”或“已看完”时，提醒会立即关闭；该逻辑已在详情页、时间表、搜索页、我的追番以及 Agent 操作中保持一致。
4. 为历史本地数据加入了提醒状态自修复逻辑，旧版本遗留的“非在看但提醒仍开启”记录会在启动恢复时自动纠正，并补充了提醒策略与 Agent 行为测试。
5. 修复了 Supabase 登录会话过期后仍被误判为已登录的问题；应用现在会在启动、回到前台和云同步前校验并尝试刷新会话，若 `JWT` 已失效则安全回到登录态，避免继续弹出 `401 Unauthorized` / `JWT expired` 的错误。
